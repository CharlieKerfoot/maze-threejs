<script lang="ts">
	...
	const handleMovement = () => {
		...
		const isPassable = (tile: Cell | Wall | undefined): boolean => {
			...
			if (tile instanceof Wall && tile.orientation === -1)
				return true;
			return false;
		};
	...
	const createMaze = (
		...
		const maze_geometry = new THREE.BoxGeometry(
			bg_width,
			bg_height,
			1,
		);
		...
		const wall_geo = new THREE.BoxGeometry(
			wall_width,
			wall_height,
			depth,
		);
		...
		for (let i = 0; i < height; i++) {
			for (let j = 0; j < width; j++) {
				...
				if (i % 2 == 1) {
					...
					if (
						left instanceof Cell &&
						right instanceof Cell
					) {
						const wall = new Wall(
							left,
							right,
		...
		const kruskal = (walls: Wall[]) => {
			const step = () => {
				...
				if (animation)
					setTimeout(step, animation_speed);
				else step();
			};
		...
		const agent_geometry = new THREE.SphereGeometry(
			wall_width / 3,
			32,
			32,
		);
